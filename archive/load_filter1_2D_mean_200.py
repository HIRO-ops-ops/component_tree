...
def extract_instances_axial(volume, area_threshold, intensity_threshold):
    """Max-treeを用いてインスタンス抽出（外枠のみ）と階層構造解析を行う"""
    H, W, D = volume.shape
    all_instances = []
    node_table = []
    tree_edges = []
    ...
    for z in range(D):
        ...
        for node in range(n_nodes):
            if node != root:
                persistence[node] = altitudes[parent[node]] - altitudes[node]

        # --- 全ノード情報を保存 ---
        for node in range(n_nodes):
            node_table.append([
                z, node, parent[node], depth[node], 
                altitudes[node], persistence[node], 
                area[node], mean_intensity[node]
            ])
            tree_edges.append([z, parent[node], node])

        # 1. まず条件（面積と明るさ）に合うノードをすべて抽出
        candidates = np.where((area > area_threshold) & (mean_intensity > intensity_threshold))[0]
        candidate_set = set(candidates)
        
        # 2. 「自分は条件に合うが、親は条件に合わない」ノードだけを厳選
        # これにより、入れ子構造の中で最も広い面積を持つ「外枠」だけが残る
        ...
        for node in selected_nodes:
            ...
            deleted_nodes[node] = False; deleted_nodes[root] = False

            node_altitudes = np.zeros(n_nodes, dtype=np.int32); node_altitudes[node] = 1
...
def save_overlay_images(instances, volume, output_dir):
    ...
    for i, inst in enumerate(instances):
        ...
        plt.title(f"Slice {z} - Instance {i+1} - Node {inst['node_id']}")
        plt.axis("off")
        save_path = os.path.join(output_dir, f"filtered_slice_{z}_instance_{i}.png")
...
def save_numpy_instances(instances, output_dir):
    ...
    os.makedirs(output_dir, exist_ok=True)
    for i, inst in enumerate(instances):
        np.save(os.path.join(output_dir, f"filtered_slice_{inst['slice']}_instance_{i}.npy"), inst["mask"])
