...
import nibabel as nib  # 追加

# ============================================
# 1. NIfTI画像の読み込みと前処理
# ============================================
nii_path = r"C:\Users\nishi\OneDrive - 近畿大学\MedICoT_Lab\BraTS\BraTS_project\HU_A0001_pt.npy"  # ★ ここにお手持ちの .nii ファイルのパスを指定してください

try:
    img = nib.load(nii_path)
    data = img.get_fdata()
except FileNotFoundError:
    print(f"エラー: {nii_path} が見つかりません。パスを確認してください。")
    exit()
...
def process_slice(z_index):
    ...
    # ★ 重要: 医療画像は float 型で微小なノイズが多く、そのままツリーを作ると
    # ノード数が爆発してPCが固まるため、0〜255の整数値(uint8)に量子化します。
    ...
    image_proc = (image_proc * 255).astype(np.uint8)  # Higra処理用画像
...
img_ax.set_title(f"NIfTI Slice {z_center}")
...
def update(val):
    ...
    img_ax.set_title(f"NIfTI Slice {z}")
